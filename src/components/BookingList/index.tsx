    ...
    <div className="space-y-3 sm:space-y-4" data-testid="booking-list">
      {bookings.map((booking, index) => (
        ...
        >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="text-lg sm:text-xl font-bold text-gray-900 truncate">{booking.name}</h3>
                  <p className="text-xs text-gray-500 hidden sm:block">Booking ID: {booking.id}</p>
                </div>
              </div>
              <div className="space-y-2 sm:space-y-3">
                <div className="flex items-center gap-2 sm:gap-3 text-sm">
                  ...
                  <span className="text-gray-700 font-medium text-xs sm:text-sm truncate">{booking.email}</span>
                </div>
                <div className="flex items-center gap-2 sm:gap-3 text-sm">
                  ...
                  <span className="text-gray-700 font-medium text-xs sm:text-sm">{formatDate(booking.date)}</span>
                </div>
                <div className="flex items-center gap-2 sm:gap-3 text-sm">
                ...
                {booking.notes && (
                  <div className="flex items-start gap-2 sm:gap-3 mt-2 sm:mt-3 p-2 sm:p-3 bg-gray-50 rounded-xl">
                    ...
                    <span className="text-xs sm:text-sm text-gray-700 flex-1 break-words">{booking.notes}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {(onEdit || onDelete) && (
            <div className="flex gap-2 sm:gap-3 pt-4 sm:pt-5 border-t-2 border-gray-100">
              {onEdit && (
                <button
