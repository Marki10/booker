...
const makeStorage = () => {
  let bookings: any[] = []
  let syncMeta = { lastSync: null as string | null, lastSyncId: null as string | null, pendingSync: false }
  return {
...
const makeApi = () => {
  return {
    ...
    createBooking: vi.fn(async (data: any) => ({ id: 'booking-remote-1', status: 'confirmed', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), ...data })),
    updateBooking: vi.fn(async (id: string, data: any) => ({ id, status: 'confirmed', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), ...data })),
...
// Access to mocks
const getStorageMock = async () => (await import('../storageService') as any).__storage
...
describe('bookingService', () => {
  ...
  it('getAllBookings returns sorted by date/time', async () => {
    ...
    storage.__setBookings([
      { id: 'booking-2', name: 'B', email: 'b', date: '2025-11-11', time: '10:00', duration: 30, status: 'confirmed', createdAt: '', updatedAt: '' },
      { id: 'booking-1', name: 'A', email: 'a', date: '2025-11-10', time: '09:00', duration: 60, status: 'confirmed', createdAt: '', updatedAt: '' },
    ])
  ...
  it('createBooking saves locally and returns booking', async () => {
    ...
    storage.__setBookings([])
    const svc = await importService()
    const created = await svc.createBooking({ name: 'C', email: 'c@e.com', date: '2025-11-12', time: '11:00', duration: 30, notes: '' })
  ...
  it('updateBooking modifies existing booking', async () => {
    ...
    const storage = await getStorageMock()
    storage.__setBookings([{ id: 'booking-1', name: 'X', email: 'x', date: '2025-11-10', time: '09:00', duration: 60, status: 'confirmed', createdAt: now, updatedAt: now }])
    const svc = await importService()
  ...
  it('deleteBooking removes booking', async () => {
    ...
    const storage = await getStorageMock()
    storage.__setBookings([
      { id: 'booking-1', name: 'X', email: 'x', date: '2025-11-10', time: '09:00', duration: 60, status: 'confirmed', createdAt: now, updatedAt: now },
    ])
    const svc = await importService()
  ...
  it('isTimeSlotAvailable detects conflicts', async () => {
    ...
    await expect(svc.isTimeSlotAvailable('2025-11-10', '09:30', 30)).resolves.toBe(false)
    // non-overlapping
    await expect(svc.isTimeSlotAvailable('2025-11-10', '10:00', 30)).resolves.toBe(true)
  })
  ...
  it('syncWithBackend merges when backend available', async () => {
    ...
    storage.__setBookings([
      { id: 'booking-1', name: 'Local', email: 'l', date: '2025-11-10', time: '09:00', duration: 60, status: 'confirmed', createdAt: '', updatedAt: '' },
    ])
    ;(api.getAllBookings as any).mockResolvedValueOnce([
      { id: 'booking-2', name: 'Remote', email: 'r', date: '2025-11-11', time: '10:00', duration: 30, status: 'confirmed', createdAt: '', updatedAt: '' },
    ])
    const svc = await importService()
