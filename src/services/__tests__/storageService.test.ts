...
describe('storageService', () => {
  ...
  it('getSyncMetadata returns defaults when missing', () => {
    ...
    expect(meta).toEqual({ lastSync: null, lastSyncId: null, pendingSync: false })
  })

  it('saveSyncMetadata persists and getSyncMetadata reads back', () => {
    const now = new Date().toISOString()
    storageService.saveSyncMetadata({ lastSync: now, lastSyncId: 'booking-1', pendingSync: true })
    const raw = window.localStorage.getItem(SYNC_META_KEY)
    expect(raw).not.toBeNull()

    const meta = storageService.getSyncMetadata()
    expect(meta).toEqual({ lastSync: now, lastSyncId: 'booking-1', pendingSync: true })
  })
