...
export const bookingService = {
  ...
  async createBooking(data: BookingFormData): Promise<Booking> {
    ...
    if (isBackendAvailable) {
      ...
      try {
        ...
        console.error('Failed to create on backend, using local booking:', error)
  ...
  // Update a booking
  async updateBooking(id: string, data: Partial<BookingFormData>): Promise<Booking | null> {
    const bookings = storageService.getBookings()
    ...
    if (isBackendAvailable) {
      ...
      try {
        ...
        const updatedBookings = bookings.map((b) => (b.id === id ? backendBooking : b))
        storageService.saveBookings(updatedBookings)
        return backendBooking
      } catch (error) {
        console.error('Failed to update on backend, using local booking:', error)
        // Continue with local booking
      }
    }
  ...
  async deleteBooking(id: string): Promise<boolean> {
    ...
    if (isBackendAvailable) {
      ...
      try {
        ...
        console.error('Failed to delete on backend, using local deletion:', error)
  ...
  async isTimeSlotAvailable(
    ...
    if (isBackendAvailable) {
      try {
        return await apiService.checkAvailability(date, time, duration, excludeBookingId)
      } catch (error) {
        console.error('Failed to check availability on backend, using local check:', error)
  ...
  // Get sync status
  getSyncStatus(): { lastSync: string | null; pendingSync: boolean; backendAvailable: boolean } {
    const metadata = storageService.getSyncMetadata()
    return {
